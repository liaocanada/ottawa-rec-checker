...
const findTimes = async url => {
    ...
    $("div .one-queue").each((i, dateElement) => {
        ...
        $("li .time-container", dateElement).each((i, element) => {
            ...
            console.log(`${time}: ${available ? "Open!!".green : "Booked".red}`);
        });
...
const formatTitle = (title, width) => {
    const leftPadding = title.length + Math.floor((width - title.length) / 2);
...
const main = async () => {
    ...
    for (const centre in centres) {
        const line = "==================================";
        console.log(line);
        console.log(formatTitle(centre, line.length));
        console.log(line);
        await findTimes(centres[centre]);
    }
    // await findTimes(centres.richcraft);
    // await findTimes(centres.cardelrec);
};
